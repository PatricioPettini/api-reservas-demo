...
class UserDetailsServiceImpTest {
    ...
    // ------------------------------------------------------------
    // TEST 1: loadUserByUsername
    // ------------------------------------------------------------
    @Test
    void loadUserByUsername_DeberiaRetornarUserDetailsConRolesYPermisos() {
        // Arrange
        when(userRepo.findUserEntityByUsername("juan")).thenReturn(Optional.of(userSec));
        ...
        verify(userRepo, times(1)).findUserEntityByUsername("juan");
    }

    @Test
    void loadUserByUsername_DeberiaLanzarExcepcionSiNoExisteUsuario() {
        // Arrange
        when(userRepo.findUserEntityByUsername("juan")).thenReturn(Optional.empty());
